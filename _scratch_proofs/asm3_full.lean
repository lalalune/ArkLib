...
-- Reduce restrictedMatchRecursionPartitionForm: push emb through B_coeff (= countPerms •
-- emb(hasseCoeffRepr𝒪) = countPerms • liftBivariate p via bridge1), and embed_W𝒪.
set_option maxHeartbeats 4000000 in
set_option linter.unusedSectionVars false in
theorem rhs_reduced (x₀ : F) (R : F[X][X][Y]) (hHyp : ClaimA2.Hypotheses x₀ R H) (t : ℕ) :
        ...
        ((∑ i1 ∈ Finset.range (t + 2),
            ∑ lam ∈ (Finset.univ : Finset (Nat.Partition (t + 1 - i1))).filter
              ...
              (liftToFunctionField (H := H) H.leadingCoeff) ^ (i1 + deltaSave i1 - 1)
                ...
                * (lam.parts.countPerms •
                    liftBivariate (H := H) (Bivariate.evalX (Polynomial.C x₀)
  ...
  refine Finset.sum_congr rfl (fun i1 _ => ?_)
  refine Finset.sum_congr rfl (fun lam _ => ?_)
  rw [embed_W𝒪, B_coeff, map_nsmul, prefactor_eq_countPerms, bridge1_honest]
...
theorem RestrictedFaaDiBrunoPartitionMatchAt_proof (x₀ : F) (R : F[X][X][Y])
  ...
  · -- Cancel countPerms and emb(partitionProd).  The residual reduces to
    --     eval₂ (T/W) p  =  (uniform W,ξ power) · eval₂ T p
    -- (equivalently  eval₂ T p = W^N · eval₂ (T/W) p  with N = natDegreeY p).
    -- This is MATHEMATICALLY FALSE for the un-cleared representative `hasseCoeffRepr𝒪`:
    -- eval₂ T p = ∑ lift(p.coeff i)·T^i  while  W^N·eval₂(T/W) p = ∑ lift(p.coeff i)·T^i·W^(N-i)
    -- differ per-term by the non-uniform power W^(N-i).  The clean uniform identity holds ONLY
    -- for the CLEARED representative `hasseCoeffRepr𝒪_cleared`
    -- (lemma `embeddingOf𝒪Into𝕃_hasseCoeffRepr𝒪_cleared`), which `B_coeff` does NOT use.
    -- Hence the keystone is NOT closeable as currently stated: `B_coeff` would need to be the
    -- cleared representative for this per-term identity to hold.
    sorry
