...
/-- The fixed-radius RS curve inputs for the probability-route Batched FRI wrappers.

This packages exactly the BCIKS20 curve theorem inputs used by the query-level
`...AndRSCurve` route: strict coefficient-polynomial residual, boundary-probability residual,
square-root-radius side condition, and the curve probability trigger. -/
def friRSCurveProbabilityInputs {t : ℕ}
...
/-- Probability-route Claim 8.3 query-lift specialized to the Batched FRI subdomain
Reed-Solomon curve correlated-agreement theorem. -/
...
/-- Raw full-domain Claim 8.2 conclusion from the Batched FRI subdomain Reed-Solomon curve
probability-route wrapper. -/
