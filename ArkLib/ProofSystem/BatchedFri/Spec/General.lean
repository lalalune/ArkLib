...
def batchedFRIOracleLens [DecidableEq F] :
  ...
  simOStmt := fun q =>
    ...
    | ⟨j, v⟩ => ReaderT.mk fun stmtIn => do
        have hv : v.1 ∈ ω.toFinset := by
          -- main-merge migration: `ω : Domain.SmoothCosetFftDomain`, so use the `Domain`
          -- `CosetFftDomainClass.*` API (mirrors the already-migrated `liftStmt` block above),
          -- not the legacy `ReedSolomon.CosetFftDomain.*` lemmas the bad merge left behind.
          simp only [CosetFftDomainClass.mem_toFinset_iff_mem]
          rcases j with ⟨j, h⟩
          have : j = 0 := by simpa using h
          simp only [Fin.coe_ofNat_eq_mod, Nat.zero_mod, Nat.reduceAdd] at v
  ...
  hEqOStmt := fun (i : Fin (k + 2)) => by
    ...
    · -- `.inl j` (j : Fin 1): the inner round-0 input oracle. The outer round-0 batched oracle 0 has
      ...
      -- `Fri.Spec.OracleStatement s ω 0 0 = (ω.subdomain 0).toFinset → F` (the index sum is empty),
      -- with `(ω.subdomain 0).toFinset = ω.toFinset`, so the two oracle families agree.
      -- main-merge migration: `ω : Domain.SmoothCosetFftDomain`, so this uses `ω.subdomain 0`
      -- (the actual `Fri.Spec.OracleStatement` domain) and the `Domain` `CosetFftDomainClass.*` API,
      -- replacing the bad merge's legacy `ReedSolomon.CosetFftDomain.subdomainNatReversed` references.
