...
theorem composedTightFull_perfectCompleteness
  ...
  -- h₈ at the welded endpoints: `tightFinalRelOut → tightFinalRelOut` via the rel-generic leaf,
  -- with h₇'s output welded in by `enrichedBinding_subset_tightFinalRelOut`.
