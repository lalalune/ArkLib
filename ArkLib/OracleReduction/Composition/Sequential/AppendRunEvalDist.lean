...
theorem appendRunRightResidualDist_holds_msg
...
`appendRunRightResidualDist_holds_msg`. This is the distribution-level keystone the LogUp
      ...
      evalDist ((P₁.append P₂).run stmt wit)
        = evalDist ((do
          ...
          return ⟨transcript₁ ++ₜ transcript₂, stmt₃, wit₃⟩) :
            OracleComp (oSpec + [(pSpec₁ ++ₚ pSpec₂).Challenge]ₒ)
              (FullTranscript (pSpec₁ ++ₚ pSpec₂) × Stmt₃ × Wit₃)) :=
  append_run_evalDist stmt wit (appendRunRightResidualDist_holds_msg stmt wit hn hDir hDir₂)

#print axioms append_run_evalDist
#print axioms appendRunRightResidualDist_holds_msg
