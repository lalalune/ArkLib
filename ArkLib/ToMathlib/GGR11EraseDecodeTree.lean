...
* `EraseDecodeTree.leafCount_le` — **GGR11 Theorem 3.6 for a genuine tree**: for
  ...
  proved by structural induction matching the Pascal recursion
...
/-- Pointwise monotonicity of the closed-form GGR11 bound in the Blue budget `b`. -/
...
/-- The closed-form GGR11 bound is monotone in the Red budget `r` once `1 ≤ L`. -/
private theorem ggr11Bound_mono_red (L : ℕ∞) (hL : 1 ≤ L) {b r r' : ℕ} (h : r ≤ r') :
    ((b + r).choose r : ℕ∞) * L ^ r ≤ ((b + r').choose r' : ℕ∞) * L ^ r' := by
  refine mul_le_mul' ?_ (pow_le_pow_right₀ hL h)
  exact_mod_cast Nat.choose_le_choose_add_right b h
...
The hypothesis `1 ≤ L` is the natural normalisation `Λ(C,δ) ≥ 1` (a code that is
non-empty / can be list-decoded has at least one close codeword); it is also what
makes the Pascal recursion `t(b,r) ≤ t(b-1,r) + L·t(b,r-1)` an *upper* bound. -/
theorem EraseDecodeTree.leafCount_le (L : ℕ∞) (hL : 1 ≤ L) :
  ...
  | node bopt rs ihb ihrs =>
      intro b r hbd hrd hbr
      -- Unpack the structural hypotheses on this node.
      obtain ⟨hbr_blue, hrs_len, hrs_red⟩ := hbr
      -- Bound the blue subtree.  Its blue depth is `< b`, red depth `≤ r`.
      have hb_node :
          (bopt.elim 0 (fun t => blueDepth t + 1)) ≤ b := by
        have := hbd
        rw [blueDepth] at this
        exact le_trans (le_max_left _ _) this
      have hr_node :
          (bopt.elim 0 redDepth) ≤ r := by
        have := hrd
        rw [redDepth] at this
        exact le_trans (le_max_left _ _) this
      -- Bound on blue subtree leaf count.
      have hblue :
          (bopt.elim 0 leafCount) ≤ ((b + r).choose r : ℕ∞) * L ^ r := by
        cases bopt with
        | none => simp
        | some t =>
            simp only [Option.elim] at hb_node hr_node hbr_blue ⊢
            -- blueDepth t + 1 ≤ b, so b = b'' + 1 with blueDepth t ≤ b''
            obtain ⟨b'', rfl⟩ : ∃ b'', b = b'' + 1 := by
              cases b with
              | zero => simp only [Option.elim] at hb_node; omega
              | succ b'' => exact ⟨b'', rfl⟩
            have hbd_t : blueDepth t ≤ b'' := by omega
            have := ihb t b'' r hbd_t hr_node hbr_blue
            calc leafCount t ≤ ((b'' + r).choose r : ℕ∞) * L ^ r := this
              _ ≤ (((b'' + 1) + r).choose r : ℕ∞) * L ^ r :=
                  ggr11Bound_mono_blue L (by omega)
      -- Each red subtree: red depth `< r`, blue depth `≤ b`, leaf count bounded.
      cases r with
      | zero =>
          -- No Red budget: every red subtree has redDepth ≤ 0 forced, but a red
          -- edge already costs 1, so there can be no red children. Show rs = [].
          have hrs_nil : rs = [] := by
            by_contra hne
            obtain ⟨t, htmem⟩ := List.exists_mem_of_ne_nil rs hne
            -- redDepth (node ...) ≥ redDepth t + 1 ≥ 1, contradicting hrd ≤ 0
            have hpos : 1 ≤ redDepth (node bopt rs) := by
              rw [redDepth]
              refine le_trans ?_ (le_max_right _ _)
              have : (redDepth t + 1) ∈ rs.map (fun s => redDepth s + 1) :=
                List.mem_map_of_mem htmem
              calc 1 ≤ redDepth t + 1 := by omega
                _ ≤ (rs.map (fun s => redDepth s + 1)).foldr max 0 :=
                    le_foldr_max _ _ this
            omega
          subst hrs_nil
          simp only [leafCount, List.map_nil, List.sum_nil, add_zero,
            Nat.add_zero, Nat.choose_self, Nat.cast_one, pow_zero, mul_one]
          simpa using hblue
      | succ r =>
          -- Bound the sum of red subtree leaf counts by `L * (b + r choose r)·L^r`.
          have hred_each : ∀ t ∈ rs,
              leafCount t ≤ ((b + r).choose r : ℕ∞) * L ^ r := by
            intro t htmem
            have hbd_t : blueDepth t ≤ b := by
              have := hbd
              rw [blueDepth] at this
              refine le_trans ?_ (le_trans (le_max_right _ _) this)
              exact le_foldr_max _ _ (List.mem_map_of_mem htmem)
            have hrd_t : redDepth t ≤ r := by
              have := hrd
              rw [redDepth] at this
              have hmem : (redDepth t + 1) ∈ rs.map (fun s => redDepth s + 1) :=
                List.mem_map_of_mem htmem
              have : redDepth t + 1 ≤ r + 1 :=
                le_trans (le_foldr_max _ _ hmem) (le_trans (le_max_right _ _) this)
              omega
            exact ihrs t htmem b r hbd_t hrd_t (hrs_red t htmem)
          have hsum :
              (rs.map leafCount).sum ≤
                (rs.length : ℕ∞) * (((b + r).choose r : ℕ∞) * L ^ r) := by
            calc (rs.map leafCount).sum
                ≤ (rs.map (fun _ => ((b + r).choose r : ℕ∞) * L ^ r)).sum := by
          ...
          have hsum' :
              (rs.map leafCount).sum ≤ L * (((b + r).choose r : ℕ∞) * L ^ r) := by
            refine le_trans hsum ?_
            exact mul_le_mul' hrs_len (le_refl _)
          -- Combine blue + red.
          calc leafCount (node bopt rs)
              = (bopt.elim 0 leafCount) + (rs.map leafCount).sum := rfl
            _ ≤ ((b + (r + 1)).choose (r + 1) : ℕ∞) * L ^ (r + 1)
                  + L * (((b + r).choose r : ℕ∞) * L ^ r) := by
                refine add_le_add hblue hsum'
            _ = ((b + (r + 1)).choose (r + 1) : ℕ∞) * L ^ (r + 1)
                  + ((b + r).choose r : ℕ∞) * L ^ (r + 1) := by ring
            _ ≤ (((b + 1) + (r + 1)).choose (r + 1) : ℕ∞) * L ^ (r + 1) := by
                rw [← add_mul]
                refine mul_le_mul' ?_ (le_refl _)
                -- (b + (r+1) choose (r+1)) + (b+r choose r) ≤ ((b+1)+(r+1) choose (r+1))
                have hkey :
                  ...
                  have e2 : b + r = b + (r + 1) - 1 := by omega
                  have e3 : b + r = b + (r + 1) - 1 + 0 := by omega
                  -- normalise `(b + r).choose r` to `(b + (r+1)).choose r`
                  have : (b + r).choose r = (b + (r + 1)).choose r := by
                    congr 1 <;> omega
                  rw [this]; ring
                exact_mod_cast hkey.le
where
  /-- A member of a list is `≤` its `foldr max`. -/
  le_foldr_max : ∀ (l : List ℕ) (n : ℕ), n ∈ l → n ≤ l.foldr max 0 := by
    intro l
    induction l with
...
theorem treeStructure_of_eraseDecodeTree
  ...
  -- Use the constant leaf-count function `t b' r' := (b'+r' choose r')·Λ^r'`.
  ...
  · intro b' r'
    -- Pascal step holds because the closed form *satisfies* the recursion (≤).
    simp only
      ...
      _ ≤ ((b' + (r' + 1)).choose (r' + 1) : ℕ∞) * (Lambda C δ) ^ (r' + 1)
          ...
          calc ((b' + (r' + 1)).choose r' : ℕ∞) * ((Lambda C δ) ^ r' * (Lambda C δ))
              = (Lambda C δ) * (((b' + (r' + 1)).choose r' : ℕ∞) * (Lambda C δ) ^ r') := by
                ring
            _ ≤ (Lambda C δ) * (((b' + 1 + r').choose r' : ℕ∞) * (Lambda C δ) ^ r') := by
                exact mul_le_mul' (le_refl _) (mul_le_mul' hcast (le_refl _))
