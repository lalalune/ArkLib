...
* `subsetSum_mem_Icc` — hence every `(k+1)`-subset sum lies in the interval `[T, U]`, `T = ∑_{j<k+1} j`,
...
there. (As analyzed in that file, breaking past this needs an exponential spread — distinct subset
sums on a Sidon domain would require `q ≳ n^{k+1}`, which cancels the gain — so the open prize lives
genuinely beyond this ceiling, via multiplicity / Guruswami–Sudan.)
...
theorem fin_val_le_strictMono {m : ℕ} {f : Fin m → ℕ} (hf : StrictMono f) :
    ∀ j : Fin m, (j : ℕ) ≤ f j := by
  intro j
  induction j using Fin.induction with
  | zero => exact Nat.zero_le _
  | succ i ih =>
    have hlt : f i.castSucc < f i.succ := hf (Fin.castSucc_lt_succ i)
    have hcs : (i.castSucc : ℕ) = (i : ℕ) := Fin.coe_castSucc i
    rw [Fin.val_succ]
    omega
...
theorem subsetSum_ge (S : Finset (Fin n)) :
  ...
  rw [hmap, ← Fin.sum_univ_eq_sum_range]
...
theorem subsetSum_mem_Icc {k : ℕ} (S : Finset (Fin n)) (hS : S.card = k + 1) (hkn : k + 1 ≤ n) :
    (∑ j ∈ Finset.range (k + 1), j) ≤ ∑ i ∈ S, (i : ℕ) ∧
      (∑ i ∈ S, (i : ℕ))
        ≤ (∑ j ∈ Finset.range n, j) - ∑ j ∈ Finset.range (n - (k + 1)), j := by
  refine ⟨?_, ?_⟩
  · have := subsetSum_ge S; rwa [hS] at this
  · -- upper bound via the lower bound on the complement
    have hpart : (∑ i ∈ S, (i : ℕ)) + ∑ i ∈ Sᶜ, (i : ℕ) = ∑ i : Fin n, (i : ℕ) := by
...
/-- The interval `[T, U]` for the arithmetic domain has exactly `(k+1)(n-k-1)+1` integers. -/
theorem Icc_length {k : ℕ} (hkn : k + 1 ≤ n) :
  ...
  rcases Nat.exists_eq_add_of_le hkn with ⟨t, ht⟩
  subst ht
  -- n = (k+1) + t ; goal becomes a polynomial identity in k, t
  have e1 : (k + 1) + t - (k + 1) = t := by omega
  rw [e1]
  rcases Nat.eq_zero_or_pos (k + 1 + t) with h0 | hpos
  · omega
  · ring_nf
...
most `(k+1)(n-k-1)+1` — the matching ceiling for the arithmetic-domain MCA witness spread. -/
theorem card_arithmeticSpread_le {k : ℕ} (hkn : k + 1 ≤ n) :
    ((Finset.powersetCard (k + 1) (Finset.univ : Finset (Fin n))).image
        (fun S => ∑ i ∈ S, (i : ℕ))).card ≤ (k + 1) * (n - 1 - k) + 1 := by
  sorry
