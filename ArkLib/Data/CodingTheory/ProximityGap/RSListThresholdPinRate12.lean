...
Companion to `RSListThresholdPin.lean` (rate `1/16`), at the **headline prize rate** `rho = 1/2`
that matches the negative-side capstone `rs_uptoCapacity_false_rate12_n256` (`delta* < 1 - rho =
1/2`).

The Sudan list-size bound degenerates at high rate (at `rho = 1/2` it yields error budget `0`), so
the rate-`1/2` lower side needs the *minimum-distance* unique-decoding bound instead. This file
proves a clean, reusable

  `reedSolomon_Lambda_le_one` -- `2 floor(delta n) < n - k + 1 ==> Lambda(RS[k], delta) <= 1`,

directly from `ReedSolomon.unique_decode` (two codewords within the unique-decoding radius of a
common word coincide, so each point list is a subsingleton). Combined with the unconditional
capacity ceiling `ProximityGap.listLatticeThreshold_le_capacity`, this pins the genuine rate-`1/2`
RS list-decoding threshold:

  `rs_ld_threshold_pin_rate12` -- for `RS[F, alpha, 128]` on a size-`256` domain, `m = 1`,
  `epsilon* = 2^{-128}`, over every field with `2^128 <= |F|`:
  `64 <= listLatticeThreshold <= 128`, i.e. `0.25 <= delta* <= 0.5` (unique-decoding radius
  `delta_min/2` to capacity `1 - rho`).

The negative-side result shows the upper end is *not attained* (`delta* < 1/2` strictly); pinning
the exact value inside `[0.25, 0.5]`, in particular whether it reaches the Johnson radius
`1 - sqrt(1/2)` or beyond, is the open prize. Nothing here is fabricated.
...
/-- **Reed-Solomon unique-decoding list cap `Lambda(RS, delta) <= 1`.** If
`2 floor(delta n) < n - k + 1` (the unique-decoding radius `delta < delta_min/2`), every point list
`Lambda(RS[k], delta, f)` is a subsingleton: by `ReedSolomon.unique_decode`, two codewords within
`floor(delta n)` Hamming errors of a common word `f` coincide, so the maximised list size is at most
`1`. -/
theorem reedSolomon_Lambda_le_one {F : Type*} [Field F] [Fintype F] [DecidableEq F]
  ...
  classical
  -- relative-distance membership implies Hamming distance at most `floor(delta n)`
  ...
  -- subsingleton implies `ncard <= 1`, hence `Lambda <= 1`
...
/-- **Concrete two-sided pin of a Reed-Solomon list-decoding threshold (rate `1/2`).** For the
rate-`1/2` Reed-Solomon code `RS[F, alpha, 128]` on a size-`256` domain, with single column `m = 1`
and prize tolerance `epsilon* = 2^{-128}`, over any field with `2^128 <= |F|`, the faithful
list-decoding lattice is nonempty and its threshold satisfies `64 <= delta*-index <= 128`
(`0.25 <= delta* <= 0.5` in relative-radius units): the unique-decoding radius `delta_min/2` lower
bounds it and the capacity radius `1 - rho` upper bounds it. The negative-side
...
theorem rs_ld_threshold_pin_rate12
  ...
  have hfloor : ⌊(((64 : ℝ≥0) / (Fintype.card (Fin 256) : ℝ≥0) : ℝ≥0) : ℝ)
    ...
    norm_num
  -- Lower side: unique-decoding cap `Lambda(RS[128], 64/256) <= 1`.
  have hLam : ListDecodable.Lambda ((ReedSolomon.code α 128 : Set (Fin 256 → F)))
    ...
    exact_mod_cast hb
  -- Budget: `ell^m = 1 <= epsilon* * |F|` because `|F| >= 2^128`.
  ...
  · -- Upper side: capacity ceiling `delta* <= n - k = 256 - 128 = 128`.
