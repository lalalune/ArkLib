...
Wires the from-scratch UDR bad-scalar bound to the actual `ε_mca` of a Reed–Solomon
code, yielding ABF26 Table-1 row 2 (`ε_mca ≤ O(δn)/|F|` below the
unique-decoding radius) with **no admit**:

  `epsMCA_rs_udr_le` — for `RS[F, α, k]` with `k ≤ n` and the regime
  `3(n − t) < n − k + 1` (`t = ⌈(1-δ)n⌉`, the witness-size floor),
  `ε_mca(RS, δ) ≤ 2(n − t)/|F|`.

Since `n − t ≈ δn`, this is `ε_mca ≤ 2δn/|F|`, a genuine positive-side
lower-witness on `δ*` (`δ* ≥ δ` whenever the regime holds). The minimum-distance
input is `ReedSolomon.code_eq_of_agree` (degree-`<k` codewords agreeing on
`≥ k` points are equal).
...
theorem badGamma_le (G : Finset F) (e₀ e₁ : ι → F)
  ...
  · intro γ hγ
    have hbad := hG γ hγ
    simp only [dif_pos hbad, coe_filter, mem_univ, true_and, Set.mem_setOf_eq]
    exact hbad.choose_spec.1
  · intro γ₁ hγ₁ γ₂ hγ₂ heq
    have hbad1 := hG γ₁ hγ₁
    have hbad2 := hG γ₂ hγ₂
    simp only [dif_pos hbad1, dif_pos hbad2] at heq
    have h1 := hbad1.choose_spec
    have h2 := hbad2.choose_spec
    rw [← heq] at h2
    exact mul_right_cancel₀ h1.1
      (by
        linear_combination h1.2 - h2.2 :
          γ₁ * e₁ hbad1.choose = γ₂ * e₁ hbad1.choose)
  ...
  · obtain ⟨γa, haG, γb, hbG, hab⟩ := hG
    ...
    have hGbad : ∀ γ ∈ G, ∃ i, e₁ i ≠ 0 ∧ e₀ i + γ * e₁ i = 0 := by
      ...
      have hcollapse : w γ = c₀ + γ • c₁ := by
        ...
        have hsub2 :
            (univ.filter (fun i => w γ i ≠ (c₀ + γ • c₁) i)) ⊆
              (T ∩ S γ)ᶜ := by
          intro i hi; simp only [mem_filter, mem_univ, true_and] at hi
...
theorem rs_min_dist (α : ι ↪ F) (k : ℕ) [NeZero k] (hk : k ≤ Fintype.card ι) :
    ∀ a ∈ (ReedSolomon.code α k : Set (ι → F)),
      ∀ b ∈ (ReedSolomon.code α k : Set (ι → F)),
...
theorem epsMCA_rs_udr_le [Fintype F]
    (α : ι ↪ F) (k : ℕ) [NeZero k] (hk : k ≤ Fintype.card ι) (δ : ℝ≥0)
    (htn : ⌈(1 - δ) * (Fintype.card ι : ℝ≥0)⌉₊ < Fintype.card ι)
    (hreg :
      3 * (Fintype.card ι - ⌈(1 - δ) * (Fintype.card ι : ℝ≥0)⌉₊) <
        Fintype.card ι - k + 1) :
  ...
  set G : Finset F :=
    univ.filter
      (fun γ : F => mcaEvent (ReedSolomon.code α k : Set (ι → F)) δ (u 0) (u 1) γ)
    with hGdef
  ...
  refine badCount_udr_le (ReedSolomon.code α k) (u 0) (u 1)
    (Fintype.card ι - k + 1) t htn hmd hreg G S w ?_ ?_ ?_ ?_
  · intro γ hγ
    rw [hGdef, mem_filter] at hγ
    have h := hγ.2
    simp only [hSdef, dif_pos h]
    have hcardR :
        (1 - δ) * (Fintype.card ι : ℝ≥0) ≤ ((h.choose.card : ℕ) : ℝ≥0) :=
      h.choose_spec.1
...
(`2(n−t)·2^128 ≤ |F|`, `t = ⌈(1-δ)n⌉`) in the unique-decoding regime, radius
`δ` certifies `ε_mca(RS, δ) ≤ ε*` (`ε* = 2^{-128}`), so the Grand MCA threshold
satisfies `δ* ≥ δ`. This upgrades the lower witness from `δ = 0` to the
unique-decoding radius `δ ≲ (1−ρ)/3`. -/
noncomputable def rs_mcaLowerWitness_udr [Fintype F]
    ...
    (hreg :
      3 * (Fintype.card ι - ⌈(1 - δ) * (Fintype.card ι : ℝ≥0)⌉₊) <
        Fintype.card ι - k + 1)
    (hF :
      2 * (Fintype.card ι - ⌈(1 - δ) * (Fintype.card ι : ℝ≥0)⌉₊) * 2 ^ 128 ≤
        Fintype.card F) :
  ...
  bound := by
    ...
    set m : ℕ :=
      2 * (Fintype.card ι - ⌈(1 - δ) * (Fintype.card ι : ℝ≥0)⌉₊)
      with hm
    ...
    have hcoe : (epsStar : ENNReal) = 1 / 2 ^ 128 := by
      rw [epsStar, ENNReal.coe_div (by positivity), ENNReal.coe_one, ENNReal.coe_pow,
        ENNReal.coe_ofNat]
    rw [hcoe]
    have hFge : ((m * 2 ^ 128 : ℕ) : ℝ≥0∞) ≤ (Fintype.card F : ℝ≥0∞) := by
      exact_mod_cast hF
    have hcast : ((m * 2 ^ 128 : ℕ) : ℝ≥0∞) = (m : ℝ≥0∞) * 2 ^ 128 := by
      push_cast
      ring
    calc (m : ℝ≥0∞) / (Fintype.card F : ℝ≥0∞)
        ≤ (m : ℝ≥0∞) / ((m * 2 ^ 128 : ℕ) : ℝ≥0∞) :=
          ENNReal.div_le_div_left hFge (m : ℝ≥0∞)
