...
/-- **`rpow` overflow bridge.** For `q > 1`, `0 < n1`, `0 < εq`,
`logb q (n1·εq) < E → εq < q^E/n1`. -/
...
theorem hover_core_n256 (q : ℝ) (hq1 : (2 : ℝ) ^ 128 ≤ q) (hq2 : q ≤ (2 : ℝ) ^ 256) :
  ...
  have h2le : (2 : ℝ) ≤ 2 ^ 128 := by
    have := pow_le_pow_right₀ (one_le_two : (1 : ℝ) ≤ 2) (by norm_num : 1 ≤ 128)
    simpa using this
  ...
  have hcorr : 128 * Real.log (q / (q - 1)) ≤ 1 := by
    ...
    calc 128 * Real.log (q / (q - 1)) ≤ 128 * (1 / (q - 1)) := by
            rw [hsimp] at hle
            linarith [mul_le_mul_of_nonneg_left hle (by norm_num : (0 : ℝ) ≤ 128)]
      _ ≤ 128 * (1 / 128) := by
            linarith [mul_le_mul_of_nonneg_left hfrac (by norm_num : (0 : ℝ) ≤ 128)]
      _ = 1 := by norm_num
  have hsplit : Real.log (257 * (q / 2 ^ 128)) =
      Real.log 257 + Real.log q - 128 * Real.log 2 := by
