...
Canonical capstone tying the lower (Johnson) and sharpened upper (overflow ⇒ list-decoding
capacity) bounds into a single *strict* two-sided trap of the genuine threshold:

  `rs_ld_threshold_gap_bracket` — for `RS[F, α, k]`, `m = 1`, given a Johnson-gap lower
  witness at index `j_lo` (with budget-clearing list cap) and a base-code overflow at
  index `j_hi`:

      `j_lo  ≤  listLatticeThreshold  <  j_hi`.

The lower side is `rs_ld_threshold_johnson_pin_general` (second-moment Johnson bound,
reaches the Johnson radius `1 − √ρ`); the strict upper side is
`listLatticeThreshold_lt_of_overflow` fed by a base-code list-size lower bound. Instantiated
with `j_lo` at the Johnson radius and `j_hi` at the capacity-exponent overflow threshold
(via `rs_lambda_gt_of_capExp_overflow`), this traps `δ*` in `[1 − √ρ, δ_LD)` with
`δ_LD = H_q⁻¹(1 − ρ)` the list-decoding capacity — i.e. strictly inside the open
Johnson→capacity gap, with the conjectured value `δ_LD` as the (excluded) upper end.

The only remaining open statement is the matching lower bound `δ* ≥ δ_LD`, which is exactly
the $1M prize. Axiom-clean (`[propext, Classical.choice, Quot.sound]`).
...
/-- **Strict two-sided bracket on the genuine list-decoding threshold.** For `RS[F, α, k]`,
single column, a Johnson-gap lower witness at `j_lo` (budget cleared by its list cap) and a
base-code overflow at `j_hi` together trap the lattice threshold strictly:
`j_lo ≤ δ* < j_hi`. -/
