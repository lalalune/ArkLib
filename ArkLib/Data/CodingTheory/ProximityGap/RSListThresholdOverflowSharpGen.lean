...
General-`m` form of `listLatticeThreshold_lt_of_overflow_fin_one`: the interleaving lower
bound `Lambda_interleaved_ge` (`Λ C δ ≤ Λ(C^⋈Fin m, δ)`) suffices — no equality needed —
to push a base-code overflow through the faithful lattice at any arity `m ≠ 0`:
...
`Λ(C, j/n) ≤ Λ(C, t/n) ≤ Λ(C^⋈Fin m, t/n) ≤ ε*·|F|` (monotonicity + interleaving
lower bound), contradicting the overflow. So all members are `< j` and `max' < j`.

Composed with the capacity-exponent overflow `rs_lambda_gt_of_capExp_overflow`, this caps
`δ*` at the list-decoding capacity `δ_LD = H_q⁻¹(1 − ρ) < 1 − ρ` for **every** column
count `m`, not just `m = 1`. Axiom-clean (`[propext, Classical.choice, Quot.sound]`).
...
/-- **Base-code overflow pushes the threshold below `j`, at every arity.** If `Λ(C, j/n)`
already exceeds the budget `ε*·|F|`, then for any `m ≠ 0` the faithful list-decoding
lattice threshold is strictly below `j`. -/
theorem listLatticeThreshold_lt_of_overflow
  ...
  have hrad : (((j : ℝ≥0) / (Fintype.card ι : ℝ≥0) : ℝ≥0) : ℝ)
    ...
    have h1 : ((j : ℝ≥0) / (Fintype.card ι : ℝ≥0)) ≤
