...
additive character turns the subset *sum* into a *product* of unit-modulus values — the multiplicative
form that meets Vieta. -/
...
*primitive* additive character `ψ : AddChar F ℂ`, any finite subset `G ⊆ F`, size `m`, and `target ∈ F`,
the count `N(m, target)` is given, as an element of `ℂ`, by the dual-group average
...
combinatorics is the elementary symmetric / generating-function coefficient (`charWeightedCount_eq_coeff`). -/
...
*trivial-character* contribution to `N(m, target)` for **every** `m` and `target` over a genuine finite
field; the remaining sum over `b ≠ 0` is the Gauss-type error. -/
...
/-- **Each character-weighted count has modulus `≤ C(|G|, m)`.** `charWeightedCount ψ b G m` is a sum
of `C(|G|, m)` products of values `ψ(b·x)`, each of modulus `1` (`AddChar.norm_apply`, finite group),
so the triangle inequality gives `‖charWeightedCount ψ b G m‖ ≤ C(|G|, m)`. -/
...
each of the `q − 1` terms has modulus `‖ψ(−b·target)·charWeightedCount‖ ≤ 1·C(|G|, m)`. This is the
*same order* as the main term `C(|G|, m)`: the bare additive-character orthogonality cannot separate
the error from the main term, so it cannot by itself decide whether `N(m, target)` is `≈ C(|G|,m)/q`
(needs `q`-fold square-root cancellation, i.e. Weil-type input using the **multiplicative** structure
of `G`, which the additive characters do not see). We state the delimiter precisely. -/
...
(`FiniteField.primitiveChar_to_Complex`), so the hypothesis `ψ.IsPrimitive` of the master identity is
satisfiable; the exact formula `subsetSumCount_eq_charSum` is therefore a genuine, non-vacuous statement
about `N(m, target)`. -/
