...
polynomially bounded products, do not suffice.

Sorry-free and axiom-clean. See `DISPROOF_LOG.md` (Loop28).
-/
...
/-- **Variable-factor fold recursion telescopes to the product of factors.** The constant-factor
Loop 24 recurrence is only a special case; with factors `a_j`, the full blowup is
`∏_{j<m} a_j`. -/
