...
`le_mcaDeltaStar_of_good` (the field-lower hypothesis clears the `2(n−t)/|F| ≤ ε*` budget); the upper
side is `rs_mcaDeltaStar_le_capacity` (the near-capacity refutation). Instantiated at the
