...
* `StrictCoeffPolysResidualShareOr ℓ T` — verbatim `StrictCoeffPolysResidualShare`,
...
`StrictCoeffPolysResidualShare`, except the conclusion allows the `jointAgreement`
...
def StrictCoeffPolysResidualShareOr {k deg : ℕ} {domain : ι ↪ F} {δ : ℝ≥0}
    (ℓ T : ℕ) : Prop :=
...
theorem strictCoeffPolysResidualShareOr_of_share
    ...
    (h : StrictCoeffPolysResidualShare (k := k) (deg := deg) (domain := domain) (δ := δ)
      ℓ T) :
    StrictCoeffPolysResidualShareOr (k := k) (deg := deg) (domain := domain) (δ := δ)
      ℓ T :=
...
#print axioms ProximityGap.StrictCoeffPolysResidualShareOr
