...
theorem gkl24_cubeRoot_mca_bound
    ...
    linear_epsMCA_1_5_johnson_gkl24 C δ_min η δ h_δ_min hη hη_lt_δ_min hδ := by sorry

/-- **[BGKS20 Lemma 3.2]** η-margin fold/interleave CA bound in the 1.5-Johnson regime. -/
theorem bgks20_etaMargin_ca_bound
    (C : ModuleCode ι F A) (δ_min η δ : ℝ≥0)
    (h_δ_min : (δ_min : ℝ) = (Code.minDist (C : Set (ι → A)) : ℝ) / Fintype.card ι)
    (hη : 0 < η) (hη_lt_δ_min : η < δ_min)
    (hδ : (δ : ℝ) ≤ 1 - ((1 - (δ_min : ℝ) + (η : ℝ)) ^ ((1 : ℝ) / 3))) :
    linear_epsCA_1_5_johnson_bgks20 C δ_min η δ h_δ_min hη hη_lt_δ_min hδ := by sorry
...
theorem bchks25_rs_epsMCA_johnson_range
    (domain : ι ↪ F) (k : ℕ) (η δ : ℝ≥0)
    (hη : 0 < η)
    (hδ : rs_epsMCA_johnson_range_condition domain k η δ) :
    rs_epsMCA_johnson_range_bchks25 domain k η δ hη hδ := by sorry
...
theorem bchks25_kk25_rs_epsCA_lower_capacity
    (c : ℝ≥0) (hc : 0 < c) (ρ : ℝ≥0) (hρ_pos : 0 < ρ) (hρ_lt : ρ < (1 / 2 : ℝ≥0)) :
    rs_epsCA_lower_capacity_bchks25_kk25 c hc ρ hρ_pos hρ_lt := by sorry
...
theorem cs25_rs_epsCA_breakdown_lower
    ...
    rs_epsCA_breakdown_cs25_entropyBallLowerWitness domain k δ hq_ge hδ_lo hδ_hi := by sorry
...
theorem bchks25_rs_epsCA_johnson_jump
    {FC : Type} [Field FC] [Fintype FC] [DecidableEq FC] [CharP FC 2]
    (ε : ℝ≥0) (hε : 0 < ε) :
    rs_epsCA_johnson_jump_bchks25 (FC := FC) ε hε := by sorry
...
theorem gg25_subspaceDesign_epsMCA
    (s : ℕ) (τ : ℕ → ℝ) (C : Submodule F (ι → Fin s → F))
    (h : IsSubspaceDesign s τ C)
    (t : ℕ) (ht : 0 < t) :
    subspaceDesign_epsMCA_gg25 s τ C h t ht := by sorry
...
theorem gg25_frs_epsMCA_capacity
    (domain : ι ↪ F) (k s : ℕ) (ω : F)
    (η : ℝ) (hη_pos : 0 < η) (hη_lt : η < 1)
    (hs_gt : (s : ℝ) > 16 / η ^ 2) :
    frs_epsMCA_capacity_gg25 domain k s ω η hη_pos hη_lt hs_gt := by sorry
