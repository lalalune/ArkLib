...
and `A(xⱼ) + yⱼ · B(xⱼ) = 0` at every point. This is the Polishchuk-Spielman / Berlekamp-Welch
existence engine: the pair-space `degreeLT d₀ × degreeLT d₁` has dimension `d₀ + d₁`, the
evaluation map to `F^N` has rank `≤ N < d₀ + d₁`, so its kernel is nontrivial. With `d₁ = e+1`
(locator `E := B`), `d₀ = k+e` (`N := -A`), and `y :=` received values, `A(αⱼ) + yⱼ E(αⱼ) = 0`
is exactly the Berlekamp-Welch key equation `E(αⱼ)·yⱼ = N(αⱼ)` - and `k + 2e + 1 > n` is the
unique-decoding condition. This is the existence half of the BCIKS20 proximity-gap bivariate
lift. -/
...
polynomial `Q(X, Z) = ∑ⱼ Qⱼ(X)·Zʲ` (each `Qⱼ` of degree `< dX+1`, not all zero) vanishing at every
grid point: `∑ⱼ Qⱼ(xₖ)·zₖʲ = 0`. The bidegree space `Fin (dZ+1) → degreeLT (dX+1)` has dimension
