...
// Parallel single-direction necklace incidence: split enumeration over the first chosen
// element f1 of `rest` (the 2nd subset element after the fixed 0). Each worker enumerates the
// remaining (s-2)-subset of [f1+1, n). Returns u64::MAX if any witness is heavy.
fn incidence_neck_par(n:usize,mua:&[u64],mub:&[u64],k:usize,p:u64,s:usize,invd:&[u64],fac:u64)->u64{
    if s<=k {return u64::MAX;}
    if s<2 {return 0;}
    // f1 ranges so that (s-2) more elements fit in (f1, n): f1 <= n-(s-1)
    let f1s:Vec<usize>=(1..=(n-(s-1))).collect();
    let results:Vec<(HashSet<u64>,bool)>=f1s.par_iter().map(|&f1|{
        let mut local:HashSet<u64>=HashSet::new();
        let mut heavy=false;
        // enumerate (s-2)-subsets of [f1+1, n)
        let m2=s-2; // count of remaining elements
        let mut comb=vec![0usize;s];
        comb[0]=0; comb[1]=f1;
        let mut u0=vec![0u64;s];let mut u1=vec![0u64;s];let mut full=vec![0u64;s];
        if m2==0{
            // subset is just {0, f1}
            if is_canonical(&comb[..s],n){
                for (j,&idx) in comb.iter().enumerate(){u0[j]=mua[idx];u1[j]=mub[idx];}
                process_subset(&comb,&u0,&u1,&mut full,k,p,invd,n,fac,&mut local,&mut heavy);
            }
            return (local,heavy);
        }
        let mut rest:Vec<usize>=((f1+1)..(f1+1+m2)).collect();
        loop{
            for j in 0..m2{comb[2+j]=rest[j];}
            if is_canonical(&comb[..s],n){
                for (j,&idx) in comb.iter().enumerate(){u0[j]=mua[idx];u1[j]=mub[idx];}
                process_subset(&comb,&u0,&u1,&mut full,k,p,invd,n,fac,&mut local,&mut heavy);
            }
            // next (s-2)-combination of [f1+1, n)
            let mut i=m2;let mut advanced=false;
            while i>0{i-=1;if rest[i]!=i+1+n-1-m2{rest[i]+=1;for j in (i+1)..m2{rest[j]=rest[j-1]+1;}advanced=true;break;}}
            if !advanced{break;}
        }
        (local,heavy)
    }).collect();
    let mut all:HashSet<u64>=HashSet::new();
    for (l,h) in results { if h {return u64::MAX;} all.extend(l); }
    all.len() as u64
}

#[inline]
fn process_subset(comb:&[usize],u0:&[u64],u1:&[u64],full:&mut[u64],k:usize,p:u64,invd:&[u64],n:usize,fac:u64,local:&mut HashSet<u64>,heavy:&mut bool){
    let s=comb.len();
    if in_rs_idx(u1,comb,k,p,invd,n){
        if in_rs_idx(u0,comb,k,p,invd,n){*heavy=true;}
    } else {
...
fn main(){
    ...
    if let Some((a,b))=single{
        ...
        println!("NECK-PAR single (a={},b={}) n={} k={} p={}:",a,b,n,k,p);
        for s in (k+2)..(n-1){
            let inc=incidence_neck_par(n,&mua,&mub,k,p,s,&invd,fac);
