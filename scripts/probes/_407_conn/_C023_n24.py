...
def esym_all(S,tm,p):
    ...
    for x in S:
        for j in range(min(tm,len(e)-1),0,-1):
            e[j]=(e[j]+e[j-1]*x)%p
    return e
# n=24 is not dyadic (24=8*3). Use n=32 (dyadic) but cap, and n=24 as a non-dyadic control.
for n,beta in ((24,3.0),):
    p=prime_1_mod_n(n,beta); g=order_n_gen(p,n)
    powg=[pow(g,i,p) for i in range(n)]
    print(f"n={n} q={p} beta={math.log(p)/math.log(n):.2f}", flush=True)
    for rho in (0.25,):
        k=int(round(rho*n))
        print(f" rho={rho} k={k}", flush=True)
        for t in range(1,n-k+1):
            a=k+t
            if math.comb(n,a)>2_500_000: 
                print(f"  t={t} a={a} SKIP comb={math.comb(n,a)}", flush=True); continue
            ...
            nb=len(img)
            print(f"  t={t} a={a} delta={1-a/n:.3f} gcd={gcd} unit={unit} #lacBad={nb} orbits={nb/unit:.2f} mult={nb%unit==0} par={'even' if t%2==0 else 'odd'} nonzero_in_img={0 not in img and nb>0}", flush=True)
