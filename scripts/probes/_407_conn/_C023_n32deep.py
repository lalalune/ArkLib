...
print(f"n={n}(dyadic 2^5) q={p} beta={math.log(p)/math.log(n):.2f}", flush=True)
for rho in (0.25,):
    k=int(round(rho*n))
    print(f" rho={rho} k={k}  (only feasible deep-window t>=16)", flush=True)
    for t in range(16,n-k+1):
        a=k+t
        if math.comb(n,a)>3_500_000: continue
        ...
        nb=len(img)
        print(f"  t={t} a={a} delta={1-a/n:.3f} gcd={gcd} unit={unit} #lacBad={nb} orbits={nb/unit:.2f} mult={nb%unit==0} par={'even' if t%2==0 else 'odd'}", flush=True)
