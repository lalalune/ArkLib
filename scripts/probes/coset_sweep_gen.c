/* Coset-free balanced A-set check, parametrized (n, target size). */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
static long long survivors = 0;
static unsigned long long visited = 0;
static int elems[64];
...
static void dfs(int cls, int rem, int nel) {
    ...
    for (int s = 0; s <= maxs; s++) {
        ...
        for (int c = 0; c < subcnt[s]; c++) {
            for (int e = 0; e < s; e++)
                elems[nel + e] = cls + NCLS * subs[s][c][e];
...
int main(int argc, char **argv) {
    ...
    dfs(0, TARGET, 0);
    printf("n=%d a=%d: configs=%llu coset-free-balanced=%lld\n", N, TARGET, visited, survivors);
    return 0;
}
