...
int main(int argc,char**argv){
  ...
  free(seen);
  double Ssum=0.0; int T=0; int started=0;
  printf("a=%d n=%lld beta=%.1f p=%lld bstar=%lld\n", a,n,beta,p,bb);
  printf("  level deficits delta_k = 1-rho_k (k=0 top .. a-1 bottom):\n  ");
  for(int k=0;k<a;k++){ ll order=1LL<<(a-k); if(order<2)break;
    ll hh=powmod(g,(p-1)/order,p);
    ...
    printf("%.4f ", delta);
    if(delta<=1e-9 && !started) T++; else started=1;
    Ssum += delta;
  }
  double log2=log(2.0);
  printf("\n  T=%d a-T=%d  Sum_delta=%.4f  budget_thresh=(log2/2)*a=%.4f  Sum<thresh? %s\n",
         T,a-T,Ssum,(log2/2)*a, (Ssum<(log2/2)*a)?"YES(route walled)":"NO");
  return 0;
}
