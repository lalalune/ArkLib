...
static long count_list(const u64* w){
    long total=0;
    // build flat list of (i0,i1) start pairs for balanced 2-level parallelism
    int pairs[4096][2], npairs=0;
    for(int i0=0;i0<=N-K;i0++) for(int i1=i0+1;i1<=N-K+1;i1++){ pairs[npairs][0]=i0; pairs[npairs][1]=i1; npairs++; }
    #pragma omp parallel reduction(+:total)
    {
        int S[64]; int idx[16];
        #pragma omp for schedule(dynamic,4)
        for(int pp=0; pp<npairs; pp++){
            int i0=pairs[pp][0], i1=pairs[pp][1];
            idx[0]=i0; idx[1]=i1; for(int m=2;m<K;m++) idx[m]=i1+(m-1);
            while(1){
                int nS=interp_agree(idx,w,S);
                if(nS>=A && is_canonical(idx,S,nS)) total++;
                int p=K-1;
                while(p>=2 && idx[p]==N-K+p) p--;
                if(p<2) break;
                idx[p]++;
