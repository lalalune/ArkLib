...
def incidence(a,b,n,mu,k,p,r):
    ...
    for R in itertools.combinations(range(n),s):
        ...
        if in_RS(u1,pts):
            if in_RS(u0,pts):return ('univ',None)
            continue
        a0=ddk(u0,pts);a1=ddk(u1,pts)
        if a1%p==0:continue
        gm=(-a0*inv(a1))%p
        if in_RS([(u0[i]+gm*u1[i])%p for i in range(s)],pts):gam.add(gm)
    return ('count',len(gam))
n=16;k=2;s=4;r=n-s
print("n=%d,k=%d,s=%d top-dir (a=n-1=%d, b=k+1=%d):"%(n,k,s,n-1,k+1))
for p in [97,193,257,449,577,1153,3329]:
    if not isprime(p) or p%n!=1: continue
    mu=setup(n,p)
    tag,val=incidence(n-1,k+1,n,mu,k,p,r)
    print("  p=%6d  (%s,%s)"%(p,tag,val))
