...
def census(q, n, k, a_exp, w):
    ...
    return ncert, gammas
for q in (97, 257):
    nc, gs = census(q, 16, 3, 9, 10)   # j=2, the 952-cert instance
    nz = [x for x in gs if x != 0]
    cosets = set(pow(x, 16, q) for x in nz)
    print(f"(q={q},16,3,w=10,s=9) j=2: cert={nc} #γ={len(gs)} #cosets={len(cosets)}", flush=True)
for q in (97, 257):
    nc, gs = census(q, 16, 2, 8, 12)   # j=1, the 65-γ instance
    nz = [x for x in gs if x != 0]
    cosets = set(pow(x, 16, q) for x in nz)
    print(f"(q={q},16,2,w=12,s=8) j=1: cert={nc} #γ={len(gs)} #cosets={len(cosets)}", flush=True)
