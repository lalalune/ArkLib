...
at the prize this exceeds Wick=(2r-1)‼·n^r for r > ~6.6 — making the hypothesis `E_r ≤ K^r·Wick`
...
print("PRIZE extrapolation (n=2^30, p=2^158): b0/Wick = n^{2r}/(p·(2r-1)‼·n^r) > 1 for r > 6.6, and at")
print("r≈ln q≈109 it is 2^(60r-158)/2^(36r) = 2^(24r-158) ≈ 2^2458 — so FULL-energy GaussianEnergyBound")
print("(E_r ≤ K^r·Wick, K=O(1)) is UNSATISFIABLE at prize depth. The house bound needs the REDUCED")
print("energy Σ_{b≠0}|η_b|^{2r} = q·rEnergy − n^{2r} (the in-tree `eta_pow_le_energyR` / `energyR`).")
