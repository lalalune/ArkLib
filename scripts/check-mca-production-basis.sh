#!/usr/bin/env bash
# Compile the production-basis proof and its two direct dependencies.
...
for module in "$prime_module" "$basis_module" "$production_module"; do
...
lake env python3 - "$mca_build_root" "$repo_root" "$production_module" <<'PY'
import os
import subprocess
import sys

build_root, repo_root, module = sys.argv[1:]
child_env = os.environ.copy()
child_env["LEAN_PATH"] = build_root + os.pathsep + child_env.get("LEAN_PATH", "")
result = subprocess.run(
    ["lean", "--root=" + repo_root, repo_root + "/" + module + ".lean"],
    env=child_env,
)
