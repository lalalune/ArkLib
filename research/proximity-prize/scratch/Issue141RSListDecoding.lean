...
def UniformPolyListSizeConjecture
...
/-- Reduction: the uniform polynomial conjecture trivially satisfies the finite existential
`RSListDecodingCapacityConjecture`. -/
theorem capacity_conjecture_of_uniform_poly
    {ι F : Type} [Field F] [Fintype F] [Fintype ι]
    (domain : ι ↪ F) (ρ η : ℝ≥0)
    (hPoly : UniformPolyListSizeConjecture domain ρ η) :
    RSListDecodingCapacityConjecture domain ρ η := by
  -- `RSListDecodingCapacityConjecture` is already known to be true by finiteness
  exact RSListDecodingCapacityConjecture_holds domain ρ η
