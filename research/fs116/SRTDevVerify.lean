...
private theorem strip_chal_lift {αH : Type}
    (impl : QueryImpl (oSpec + fsChallengeOracle StmtIn pSpec) (StateT σ ProbComp))
    (Y : OracleComp (oSpec + fsChallengeOracle StmtIn pSpec) αH) :
    simulateQ (impl.addLift challengeQueryImpl)
        (monadLift Y : OracleComp ((oSpec + fsChallengeOracle StmtIn pSpec) +
          [(Reduction.FiatShamirProtocolSpec (pSpec := pSpec)).Challenge]ₒ) αH)
      = simulateQ impl Y := by
  convert simulateQ_addLift_liftM (pSpec := Reduction.FiatShamirProtocolSpec (pSpec := pSpec))
    impl Y using 2
  induction Y using OracleComp.inductionOn with
  | pure x => rfl
  | query_bind t k ih => simp [monadLift_bind, ih]
...
theorem ksCollapse
    ...
    (P : Prover (oSpec + fsChallengeOracle StmtIn pSpec) StmtIn WitIn StmtOut WitOut
      (Reduction.FiatShamirProtocolSpec (pSpec := pSpec)))
    (V : Verifier oSpec StmtIn StmtOut pSpec) (stmtIn : StmtIn) (witIn : WitIn) :
    simulateQ (impl.addLift challengeQueryImpl)
      ((Reduction.runWithLog stmtIn witIn { prover := P, verifier := V.fiatShamir } >>=
        fun d => liftM (fiatShamirStraightlineExtractorOfStateRestoration srExtractor stmtIn
            d.1.1.2.2 d.1.1.1 default default) >>=
          fun ewi => pure (stmtIn, ewi, d.1.2, d.1.1.2.2)).run)
    = simulateQ impl
      ((fiatShamirAdversaryExecution P V stmtIn witIn >>=
        fun r => liftM (fiatShamirStraightlineExtractorOfStateRestoration srExtractor stmtIn
            r.1.2.2 r.1.1 default default) >>=
          fun ewi => pure (stmtIn, ewi, r.2, r.1.2.2)).run) := by
  ...
  intro x
  cases x with
  | none => simp only [Option.map_none, Option.elim_none, simulateQ_pure]
  | some d =>
      simp only [Option.map_some, Option.elim_some]
      simp only [fiatShamirStraightlineExtractorOfStateRestoration_apply,
        liftComp_eq_liftM, bind_assoc, pure_bind, monadLift_bind, monadLift_pure,
        ...
        simulateQ_bind, simulateQ_map, simulateQ_pure, simulateQ_addLift_liftM,
        OptionT.simulateQ_addLift_liftM, Option.getM_map_run, Option.elimM,
        simulateQ_option_elim, simulateQ_getM_run_some, OptionT.simulateQ_getM_some,
