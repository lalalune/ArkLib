...
| `L6.10` | Soundness of Construction 6.9 | present-but-incomplete | `ToyProblem.SimplifiedIOR.simplifiedIOR_knowledgeSound` in [Spec/SimplifiedIOR.lean](../../../ArkLib/ProofSystem/ToyProblem/Spec/SimplifiedIOR.lean) | same | Stated against `Verifier.knowledgeSoundness` with `relIn := ToyProblem.Spec.outputRelationFor encode δ` (= `R̃²_{C,δ}`) and `relOut := ToyProblem.SimplifiedIOR.outputRelationFor encode δ` (= `R̃¹_{C,δ}`). Knowledge error is the **concrete** `ε_mca(C,δ) + \|Λ(C^{≡2},δ)\|/\|F\|` (no `(1−δ)^t` term; de-vacuified 2026-05-31). Carries `δ < δ_min(C)` + `[Nonempty ι]` (2026-06-02). **`paper-proof-owed`** (ABF26's OWN §6.4 result, the 1-round form of L6.8; re-tagged 2026-06-02); sorry on the proof only. |
