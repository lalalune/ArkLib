...
The [profile-boundary follow-up](astra_profile_boundary_2026-09-04.md)
uses the exact leading form at minimum contact weight to sharpen the local
order cap and derive stronger node counts. It also excludes the uniform
order-34 profile for sufficiently low-degree received lines by forcing a
polynomial graph factor. Higher-degree and nonuniform cases remain open.
