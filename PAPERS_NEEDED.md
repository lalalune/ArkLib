...
| B2 — `ProximityGap.Frontier.CurveDecodability` (currently a stub `example : True`) | [GG25] **Definition 3.1 (curve decodability)** verbatim: a code is curve-decodable if low-degree curves through close words are themselves close, with a controlled list size — exact degree/list-size/closeness quantifiers needed before any `def CurveDecodable` is written. The honesty contract forbids guessing the definition. Once the def lands, the [Jo26] Thm 4.2 curve half (generator-factor bound) becomes formalizable. | [GG25] IACR ePrint **2025/2054**. **NOT on disk** (verified 2026-06-13: `~/papers/arklib/` is empty on this checkout). Needed to unblock the B2 curve-decodability lane; until obtained the residual stays a stub, not a guessed definition. |
