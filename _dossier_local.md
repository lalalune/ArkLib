...
## 29. SELF-CORRECTIONS (session 20) + the regime-split weld — the honest frontier, machine-checked

Two corrections per the red-team discipline, then the structural landing:

**(a) §28(b) RETRACTED-AS-STATED.** "The deployed asymptotic reduces in machine-checked Lean to
`N ≪ |G|^{3/2}`" was an overclaim. Machine-checked: `E(G) = |G|·N` (homogeneity), `N ≤ |G|²`, the
Mathlib `addEnergy` bridge, and the average-side frequency counts
`#{b : ‖η_b‖² ≥ q} ≤ min(|G|, E(G)/q, |G|³/q)`. NOT machine-checked: any implication from
`N ≪ |G|^{3/2}` to `InteriorCeiling` — the only in-tree consumers of `InteriorCeiling` are
`KKH26DeltaStarReduction` and `RegimeIIBridge`; the energy chain terminates at frequency counting.
§25's average→worst-case no-go applies: even with HBK `E(G) ≪ |G|^{5/2}` formalized, no existing
route converts it to the worst-case list bound. Sharper still: at deployed scale `n ≪ √q`, the
trivial bound `‖η_b‖ ≤ n` already beats both the Weil scale `√q` and the Johnson threshold — the
whole frequency-counting layer is VACUOUS at deployed parameters (no frequency can reach Johnson
scale), so the §24–25 apparatus is a small-`q` story. §28(a) phrasing also tightened: the n=16
result is a two-sided LIST-SIZE bracket `5 ≤ |Λ| ≤ 120` (exact value 19) at one interior radius,
not a δ* point-pin.

**(b) The swarm's "JohnsonDischargeStatement ⟹ the exact δ* pin" arrow corrected** (posted on
#357). `JohnsonDischargeStatement` quantifies only over `δ < gs_johnson k n m₀` (strictly below
Johnson); `InteriorCeiling` demands goodness up to the KKH26 jump `1 − r/2^μ`. The jump exceeds
Johnson iff `r²m < 2^μ((r−2)m+1)` — true for ALL `r ≥ 4`, and for `r = 3` except the single corner
`μ = 3, m ≥ 8`, where the KKH26 count (32) sits far below the Hab25 budget (`≳ 10⁵·n/q`) so no
admissible `ε*` exists. Hence at every live parameter point, regime III is nonempty and untouched
by the Johnson lane.

**(c) LANDED: `KKH26RegimeSplit.lean`** — `powDomain` + `evalCode_eq_reedSolomon` (the first
in-tree bridge from the KKH26 ceiling family to `ReedSolomon.code`, connecting it to the entire
Hab25/BCIKS20 cone) · `RegimeIIIGoodness` (THE named open core: goodness on `[gs_johnson, jump)`)
· `interiorCeiling_of_below_and_regimeIII` (the case split) · `epsMCA_evalCode_le_of_johnsonDischarge`
(the swarm's endpoint transported to the KKH26 code) · `kkh26_deltaStar_pin_of_johnsonDischarge_and_regimeIII`
(THE WELD: `JohnsonDischargeStatement ⊕ RegimeIIIGoodness ⊕ budget arithmetic ⟹ δ* = 1 − r/2^μ`
exactly) · `gs_johnson_lt_jump` (the formal guard: regime III nonempty under `r²m < 2^μ·k`).
The prize chain now reads `CellPackageSupply → JohnsonDischargeStatement ⊕ RegimeIIIGoodness → pin`,
with the split enforced by types — no tracker arrow can silently skip the 25-year wall again.
