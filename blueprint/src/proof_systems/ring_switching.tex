...
\begin{definition}[Ring-switching profile]
  ...
  \begin{itemize}
    ...
    \item a commutative \emph{pack/trace carrier} $A$ that is an $L$-algebra (the type sent on the
      wire);
    \item two ring homomorphisms $\varphi_0, \varphi_1 : L \to A$ (the \emph{column} and \emph{row}
      embeddings);
    ...
    \item subject to the two \emph{reconstruction laws}
      \begin{align}
        z &= \textstyle\sum_{u} \varphi_0(\mathrm{decomposeRows}(z)_u)\cdot \varphi_1(\beta_u),
          \label{eq:rs_rows}\\
        z &= \textstyle\sum_{v} \varphi_1(\mathrm{decomposeColumns}(z)_v)\cdot \varphi_0(\beta_v)
          \label{eq:rs_cols}
      \end{align}
      for all $z \in A$.
  \end{itemize}
...
Laws \eqref{eq:rs_rows}--\eqref{eq:rs_cols} are the e\~q/trace structural identity of
\cite[\S2.5]{DP24}: they pin the two coordinate maps to $\varphi_0,\varphi_1,\beta$ and are precisely
what the completeness and soundness proofs consume. They also rule out degenerate profiles (e.g.
$\mathrm{decomposeColumns} \equiv 0$), so the protocol's security statements are non-vacuous.

\begin{definition}[Binius instance]
  \lean{RingSwitching.binaryTowerProfile}
  \uses{def:ring_switching_profile, def:binary_tower_field}
  \label{def:binary_tower_profile}
  For binary-tower fields $B = K$, $L$ a degree-$2^\kappa$ extension, the profile takes the
  ...
  $L$-module structures on $A$. Laws \eqref{eq:rs_rows}--\eqref{eq:rs_cols} hold by
  \texttt{Basis.sum\_repr} on these bases and are proven in ArkLib.
\end{definition}
...
\begin{theorem}[Perfect completeness]
  ...
  prover's folded $\hat s$ satisfies the verifier's column check by reconstruction law
  \eqref{eq:rs_cols}.
\end{theorem}
...
\noindent The leaf completeness and soundness proofs are in progress; the statements above are
final and instance-independent, and instantiate to end-to-end Binius security via
Definition~\ref{def:binary_tower_profile}.
